...
const string strMessageMagic = "Evarians Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	...
	if (pindexBest->nHeight == 1)
		nSubsidy = 30017250 * COIN;  // 25.01% Premine	
	else if (pindexBest->nHeight <= FAIR_LAUNCH_BLOCK) // Block 500 Instamine prevention
	        nSubsidy = 1 * COIN/2;	 // 250 EVS for instamine prevention
	else if (pindexBest->nHeight <= 1000000) // 34,982,500 EVS
		nSubsidy = 35 * COIN;	
	else if (pindexBest->nHeight <= 2000000) // 30,000,000 EVS
		nSubsidy = 30 * COIN;
	else if (pindexBest->nHeight <= 3000000) // 25,000,000 EVS 
		nSubsidy = 25 * COIN;		
    else if (pindexBest->nHeight > LAST_POW_BLOCK) // Block 3m
		nSubsidy = 0; // PoW Ends ~ 660,000,000 Total EVS Mined via PoW
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Evarians", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "26/JAN/2018 EVARIANS LAUNCH SUPPORTED BY THE EVACORP";
        CTransaction txNew;
        txNew.nTime = 1516871098;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nTime    = 1516871098;
        block.nVersion = 1;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
	block.nNonce   =  1061026;
	if(fTestNet)
        {
            block.nNonce   = 125552;
        }
	...
	block.print();
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x7afa5deda9597ef833df857245265b3ce152bfb85ddbee6a56effb5b4f4981ae"));
        block.print();
