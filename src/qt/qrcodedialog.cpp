    ...
    address(addr)
{
    ...
    if (label.trimmed().isEmpty()) {
        setWindowTitle(QString("Evarians - Generate QR Code"));
    } else {
        setWindowTitle(QString("Evarians - Generate QR Code for %1").arg(label));
    }
...
{
    QString ret = QString("evarians:%1").arg(address);
    int paramCount = 0;
